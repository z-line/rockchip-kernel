...
void C2HTxRPTHandler_8723d(PADAPTER padapter, u8 *pdata, u8 len)
{
	#define	GET_8723D_C2H_TC2H_APREQ_TXRPT_MACID1(_Header)			LE_BITS_TO_1BYTE((_Header + 0), 0, 8)
	#define	GET_8723D_C2H_TC2H_APREQ_TXRPT_TXOK1(_Header)			LE_BITS_TO_2BYTE((_Header + 1), 0, 16)
	#define	GET_8723D_C2H_TC2H_APREQ_TXRPT_TXFAIL1(_Header)			LE_BITS_TO_2BYTE((_Header + 3), 0, 16)
	#define	GET_8723D_C2H_TC2H_APREQ_TXRPT_INIRATE1(_Header)		LE_BITS_TO_1BYTE((_Header + 5), 0, 8)
	...
	*pdata, *(pdata+1), *(pdata+2), *(pdata+3), *(pdata+4), *(pdata+5), *(pdata+6), *(pdata+7));
	...
	u16 TxOK = 0, TxFail = 0;
	struct sta_priv	*pstapriv = &(GET_PRIMARY_ADAPTER(padapter))->stapriv, *pstapriv_original = NULL;
	...
	adapter_ognl = rtw_get_iface_by_id(GET_PRIMARY_ADAPTER(padapter), pstapriv->c2h_adapter_id);
	if(!adapter_ognl) {
	...
	macid = GET_8723D_C2H_TC2H_APREQ_TXRPT_MACID1(pdata);
	TxOK = GET_8723D_C2H_TC2H_APREQ_TXRPT_TXOK1(pdata);
	TxFail = GET_8723D_C2H_TC2H_APREQ_TXRPT_TXFAIL1(pdata);
	IniRate = GET_8723D_C2H_TC2H_APREQ_TXRPT_INIRATE1(pdata);
...
void C2HSPC_STAT_8723d(PADAPTER padapter, u8 *pdata, u8 len)
{
	#define	GET_8723d_C2H_SPC_STAT_TYPEB_RETRY1(_Header)			LE_BITS_TO_2BYTE((_Header + 3), 0, 16)

	RTW_INFO("%s, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", __func__,
	*pdata, *(pdata+1), *(pdata+2), *(pdata+3), *(pdata+4), *(pdata+5), *(pdata+6), *(pdata+7));

	_irqL	 irqL;
	struct sta_priv	*pstapriv = &(GET_PRIMARY_ADAPTER(padapter))->stapriv;
	struct sta_info *psta = NULL;
	_list	*plist, *phead;
	PADAPTER	adapter_ognl = NULL;
	...
	psta->sta_stats.tx_retry_cnt = GET_8723d_C2H_SPC_STAT_TYPEB_RETRY1(pdata);
	rtw_sctx_done(&pstapriv->gotc2h);
}
...
s32 c2h_handler_8723d(_adapter *adapter, u8 id, u8 seq, u8 plen, u8 *payload)
{
	...
	case C2H_AP_REQ_TXRPT:
		...
		C2HTxRPTHandler_8723d(adapter, payload, plen);
		break;
	case C2H_SPC_STAT:
		RTW_INFO("[C2H], C2H_SPC_STAT!!\n");
		C2HSPC_STAT_8723d(adapter, payload, plen);
	default:
		ret = _FAIL;
		break;
	}
